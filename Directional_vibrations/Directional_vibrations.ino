...
const int vertical_groups = 3;               // Number of motor groups
const int vertical_motorsPerGroup = 2;
const int horizontal_groups = 2;             
const int horizontal_motorsPerGroup = 1;       // Motors in each group

const int pulseDuration = 300;      // Duration of vibration pulse (milliseconds)
const int delayBetweenGroups = 1000; // Delay between motor groups (milliseconds)
...
void controlMotors_right(const int pins[][1], int numGroups, int motorsPerGroup, int speed, int pulseTime) {
  for (int group = 0; group < numGroups; group++) {
    for (int motor = 0; motor < motorsPerGroup; motor++) {
...
void controlMotors_left(const int pins[][1], int numGroups, int motorsPerGroup, int speed, int pulseTime) {
  for (int i = numGroups - 1; i >= 0; i--) {
    for (int motor = 0; motor < motorsPerGroup; motor++) {
      analogWrite(pins[i][motor], speed); 
